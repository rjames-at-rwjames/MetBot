# To plot scatter plots which show relationship between Congo OLR and TTTs
# part a - nTTT
# part b - % TTT west of 55E
...
# Info for each plot
# this is a relic from when plot had 2 doms
# 0 is continental domain
# part a
dom_a=0
# part b
dom_b=0
...
if future:
    ...
    figdir=botdir+"histpaper_v2_figs/scatter_congoOLR/"
...
for t in range(nthresh):
    ...
    for d in range(ndset):
        ...
        for mo in range(nmod):
            ...
            if thcnt > 0:
                ...
                # Now that I changed this fig it doesn't actually loop!
                doms = [dom_cont]
                ndoms = len(doms)
                wlon_picks=[contdom_wlon]
                elon_picks=[contdom_elon]
                seas_picks=[seas_cont]

                convmn_doms=np.zeros(ndoms,dtype=np.float32)
                nttts_doms=np.zeros(ndoms,dtype=np.float32)
                perttts_doms=np.zeros(ndoms,dtype=np.float32)
                ...
                xvals[cnt,fgn]=convmn_doms[dom_a]
                yvals[cnt,fgn]=perttts_doms[dom_a]
    ...
    for fg in range(len(figlabels)):
        ...
        if fg==0:
            ...
            xlab='Mean '+seas_cont+' OLR in southern Congo'
            ylab='Percentage of '+seas_cont+' TTTs west of '+str(contdom_elon)
            ax.set_xlim(190,260)
            ax.set_ylim(10,100)
    ...
    scatterfig=figdir+'/scatter_congoOLR_2panel.'+globv+'.a.'+seas_cont+'.'\
