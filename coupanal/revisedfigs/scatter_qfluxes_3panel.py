# To plot scatter plots which show relationship between qflux and TTTs
# part a - Froude number and TTT location
# part b - Froude number and TTT intensity
# part c - Angola Low and TTT intensity
...
figlabels=['a','b','c']
nplot=len(figlabels)
labpos=np.array([(0.03,0.95),(0.31,0.95),(0.59,0.95)])
...
if future:
    ...
    figdir=botdir+"histpaper_v2_figs/scatter_qfluxes/"
...
for t in range(nthresh):
    ...
    ax = plt.subplot(111)
    xvals = np.ma.zeros((nallmod,3), dtype=np.float32)
    yvals = np.ma.zeros((nallmod,3), dtype=np.float32)
    cnt = 0
    ...
    for d in range(ndset):
        ...
        for mo in range(nmod):
            ...
            if thcnt > 0:
                ...
                ind_4_x=np.zeros(ndom,dtype=np.float32)
                intensitys=np.zeros(ndom,dtype=np.float32)
                perttts=np.zeros(ndom,dtype=np.float32)


                for pt in range(ndom):
                    print 'Making calculations for dom '+dom_picks[pt]
                ...
                print 'Now we have calculated everything for 2 domains, entering 3 plots'
                ...
                yvals[cnt, fgn] = perttts[dom_no]
                if ind_4_x[dom_no]!=0:
                    xvals[cnt,fgn]=ind_4_x[dom_no]
                ...
                yvals[cnt, fgn] = intensitys[dom_no]
                if ind_4_x[dom_no]!=0:
                    xvals[cnt,fgn]=ind_4_x[dom_no]
    ...
    plt.subplots_adjust(left=0.05, right=0.85, top=0.90, bottom=0.2, wspace=0.2, hspace=0.5)
    ...
    scatterfig=figdir+'/scatter_qflux_3panel.'+under_of+'.a.'+ind_picks[0]+'_'+seas_a+'.'+dom_a+'.'\
               +'b.'+ind_picks[0]+'_'+seas_a+'.'+dom_a+'.c.'+ind_picks[1]+'.'+seas_b+'.w_'+dom_b+'.'\
               +figsuf+'.thresh_'+thnames[t]+'.png'
    print 'saving figure as '+scatterfig
    plt.savefig(scatterfig,dpi=600)
    plt.close()