...
#manntest=False
manntest=True
setto='mask' # set to 'zero' or 'mask'
...
if future:
    ...
    figdir=botdir+"/histpaper_v2_figs/comp_qflux/"
my.mkdir_p(figdir)
            ...
            for lo in range(len(edays)):
                ...
                for d in range(ndset):
                    ...
                    for mo in range(nmod):
                        ...
                        smpfile_u = botpath+'/samples/ncfiles/'+ name + '.' + name2 + '.' + globv1 + '.sampled_days.' \
                                  + sample + '.' + type + '.' + thname + '.day_'+str(edays[lo])+'.nc'

                        smpfile_v =botpath+'/samples/ncfiles/'+ name + '.' + name2 + '.' + globv2 + '.sampled_days.' \
                                  + sample + '.' + type + '.' + thname + '.day_'+str(edays[lo])+'.nc'

                        if variable == 'qflux':
                            smpfile_q = botpath+'/samples/ncfiles/'+ name + '.' + name2 + '.' + globv3 + '.sampled_days.' \
                                  + sample + '.' + type + '.' + thname + '.day_'+str(edays[lo])+'.nc'

                        if pluscon:
                            botpath_c = botdir + dset3 + '/' + name3

                            smpfile_c = botpath_c+'/samples/ncfiles/'+ name + '.' + name3 + '.' + globv_c + '.sampled_days.' \
                                  + sample + '.' + type + '.' + thname + '.day_'+str(edays[lo])+'.nc'
                        ...
                        if manntest:
                            ...
                            # Open all file
                            allfile_u = botpath+ '.' + globv1 + '.day.mean.' + ys + '.nc'

                            allfile_v = botpath+ '.' + globv2 + '.day.mean.' + ys + '.nc'

                            if variable == 'qflux':
                                allfile_q = botpath+ '.' + globv3 + '.day.mean.' + ys + '.nc'

                            if pluscon:
                                allfile_c =botpath_c+ '.' + globv_c + '.day.mean.' + ys_c + '.nc'


                        if os.path.exists(smpfile_c):
                            ...
                            if manntest:
                                ...
                                # Get the dates/indices for the sample, and other (not sample)
                                sinds = []
                                odts = []
                                oinds = []
                            ...
                            if ctyp == 'anom_mon' or ctyp == 'anom_seas':
                                ...
                                meanfile_u=botpath + '/'+ name2 + '.' + globv1 + '.mon.mean.' + ysclim + '.nc'

                                meanfile_v=botpath + '/'+ name2 + '.' + globv2 + '.mon.mean.' + ysclim + '.nc'

                                if variable == 'qflux':
                                    meanfile_q = botpath + '/'+ name2 + '.' + globv3 + '.mon.mean.' + ysclim + '.nc'
                            ...
                            if pluscon:
                                if ctyp_con == 'anom_mon' or ctyp_con=='anom_seas':
                                    meanfile_c =botpath_c + '/'+ name3 + '.' + globv_c + '.mon.mean.' + ysclim_c + '.nc'
                            ...
                            if manntest:
                                ...
                                if not fdr:
                                    ...
                                    if setto=='mask':
                                        mask=np.ma.masked_where(mask_pvals<1,mask_pvals)
                            ...
                            if ctyp=='abs':
                                ...
                                if manntest:
                                    if setto=='zero':
                                        ...
                                        mwmask_comp_u = np.ma.masked_where(np.ma.getmask(mask),compdata_u)
                                        mwmask_comp_v = np.ma.masked_where(np.ma.getmask(mask),compdata_v)
                            ...
                            if ctyp=='anom_mon' or ctyp=='anom_seas':
                                if agtest:
                                    ...
                                    if setto=='zero':
                                        ...
                                        mwmask_comp_u = np.ma.masked_where(np.ma.getmask(mask),anom_comp_u)
                                        mwmask_comp_v = np.ma.masked_where(np.ma.getmask(mask),anom_comp_v)
                                ...
                                if ctyp_con =='anom_mon' or ctyp_con=='anom_seas':
                                    ...
                                    if manntest:
                                        ...
                                        if setto=='zero':
                                            ...
                                            mwmask_comp_c = np.ma.masked_where(np.ma.getmask(mask_c), anom_comp_c)
                            ...
                            if dset=='noaa':
                                ...
                                pltname=name
                            if dset=='noaa':
                                titname='NCDR-OLR | '+labname
                            else:
                                titname=labname
                            plt.title(titname, fontsize=8,fontweight='demibold')
                            ...
                            print smpfile_c + "does not exist for this model " + name
                            print 'Leaving a gap'
                ...
                compname = figdir + 'multi_comp_'+cstr+'.'+sample+'.' + type + '.' + vfname + \
                          '.'+choosel+'.'+sub+'.'+str(int_res)+'.skip'+str(skip)+'.day_'+str(edays[lo])+'.'+thname+''+suf+'.png'
                plt.savefig(compname, dpi=600)
                plt.close()
