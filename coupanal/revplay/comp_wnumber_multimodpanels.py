...
#manntest=False
manntest=True
setto='mask' # set to 'zero' or 'mask' or 'hatch'
...
figdir=botdir+"/revplay/comp_wnumber/"
my.mkdir_p(figdir)
    ...
    for a in range(len(ctyps)):
        ...
        for o in range(len(wcb)):
            ...
            for lo in range(len(edays)):
                    ...
                    for mo in range(nmod):
                        ...
                        if os.path.exists(smpfile):
                            ...
                            if ctyp=='anom_mon' or ctyp=='anom_seas':
                                ...
                                if manntest:
                                    if setto == 'zero':
                                        mwzero_comp = anom_comp * mask_pvals

                                        data4plot = mwzero_comp[:]

                                    elif setto == 'mask':

                                        mw_mask_comp = np.ma.masked_where(np.ma.getmask(mask),anom_comp)

                                        data4plot = mw_mask_comp[:]
                            ...
                            if wplotdraw == 'all':
                                ...
                                if cnt == 1:
                                    ...
                                    m = blb.AfrBasemap2(lat, lon, latsp, lonsp, drawstuff=False, prj='cyl',
                                                        rsltn='l', \
                                                        fontdict={'fontsize': 8, 'fontweight': 'demibold'})
                            ...
                            if ctyp == 'anom_mon' or ctyp == 'anom_seas':
                                if agtest:
                                    hatch = m.contourf(plon, plat, mask_zeros, levels=[-1.0, 0.0, 1.0],
                                                       hatches=["", '.'], alpha=0)
                                if manntest:
                                    if setto=='hatch':
                                        hatch = m.contourf(plon, plat, mask_pvals, levels=[-1.0, 0.0, 1.0],
                                                   hatches=["", '.'], alpha=0)
                ...
                if manntest:
                    cstr = cstr + 'manntest_' + str(alphaFDR)+'_'+setto
                ...
                compname = figdir + 'multi_comp_' + cstr + '.' + sample + '.' + type + '.' + globv + \
                           '.' + choosel + '.' + sub + '.day_' + str(edays[lo]) + '.' + thname + '.'+figsuf+'.png'
                print
                'saving figure as ' + compname
