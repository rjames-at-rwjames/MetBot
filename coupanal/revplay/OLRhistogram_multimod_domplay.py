...
# to generate plots with distributions or thresholds
#
# options for output
...
### Running options
sub="SA_EX"    # SA = standard domain used for OLR thresholding
                # SA_ST = same but subtropics
                # SA_TROP = same but  tropics
                # SA_EX = extratropical part
threshcalc=False        # set false if only want histogram
...
figdir=bkdir+"/revplay/OLRhists_multimod"
my.mkdir_p(figdir)
...
if histplot:
    ...
    if future:
        ...
        rawfig=figdir+'/olr_raw_hist.'+dsetstr+'.'+sub+'.png'
    plt.savefig(rawfig)
