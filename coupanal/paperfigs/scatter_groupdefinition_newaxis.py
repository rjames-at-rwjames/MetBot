...
for t in range(nthresh):
    ...
    for d in range(ndset):
		...
		yvals[cnt,do]=float(len(dates_ln))/len(dates_se)*100.0
...
#
  #                  print 'Selecting rain under TTTs'
   #                 masked_rain = np.ma.zeros((nttt, nlat, nlon), dtype=np.float32)
	...
	#			ind=ix[0]
	#		    elif len(ix) > 1:
	#			ind=ix[0][0] # if dates have more than 1 CB then 2 matches will occur
					     # 	we arbitrarily select one, as it will just be used to select
					     #  raw data for the chmask which is specific to this cb 
         #                   this_dt_rain=ttt_rain_dates[ind]
          #                  print 'Checking dates correspond'
           #                 print this_dt
            #                print this_dt_rain
            #                r = np.ma.MaskedArray(rainsel[ix, :, :], mask=~chmask)
            #                masked_rain[rdt, :, :] = r
            #            else:
