...
prdir=botdir+"bar_change_percent_rain_TTTs/"
my.mkdir_p(prdir)
        ...
        for t in range(len(tlist)):
            ...
            for d in range(ndset):
                ...
                for mo in range(nmod):
                    ...
                    # Get a timeseries of total TTT rain from each event
                    ...
                    # Get historical sum value
                    ttt_pr_hist=np.nansum(reg_ttt)

                    # Get historical % value
                    percent_ttt_rain=ttt_pr_hist/pr_sum_hist * 100.0
                    ...
                    if os.path.exists(futfile):
                        ...
                        # Get timmean
                        pr_sum_fut = np.nansum(reg_all_fut)
                        ...
                        if os.path.exists(futsy):
                            ...
                            # Get fut value
                            ttt_pr_fut = np.nansum(reg_ttt_fut)

                            # Get fut % value
                            percent_ttt_rain_f = ttt_pr_fut / pr_sum_fut * 100.0

                            # Calculate change
                            ttt_change=percent_ttt_rain_f-percent_ttt_rain
            ...
            if inorder:
                indsort=np.argsort(ttt_pr)
                val4plot=ttt_pr[indsort]
            ...
            plt.ylabel('Change in percent rain from TTTs',fontweight='demibold')


            ### Finalising plot
            print 'Finalising plot'
            ### Naming plot
            figsuf = sub + '.undereach.'+thname
            if test_scr2:
                figsuf = figsuf + '.test_scr2'

            figname=prdir+'Barchart_change_percen_rain_TTTs'+figsuf+'.'+dom+'.'+tname+'.png'
